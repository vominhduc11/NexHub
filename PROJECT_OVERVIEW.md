...
|---------|------|----------|--------------|------------------|---------|
| **Auth Service** | 8081 | nexhub_auth | RSA-256 JWT with JWKS, RBAC, Account management, Reseller registration, Enhanced ResellerEventService with dual Kafka event processing, Cross-service audit trails, Dual consumer configuration | Spring Security, JJWT, JPA, Kafka Producer/Consumer (dual consumers), OpenFeign, nexhub-common, BaseControllerAdvice | ✅ Production Ready |
| **User Service** | 8082 | nexhub_user | Customer & Reseller CRUD with soft delete/restore, Profile management, Account integration, Dual event publishing (deletion/restoration), Cross-service validation endpoints (existence checks only) | Spring Data JPA, Redis caching, MapStruct mapping, Kafka Producer (dual topics), nexhub-common, BaseControllerAdvice | ✅ Production Ready |
...
- `nexhub_user` - Customer and reseller profiles, event-driven data
- `nexhub_notification` - Notification records, dealer registrations, audit trails
