...
// @RestControllerAdvice(basePackages = "com.devwonder.warranty_service.controller")
@Order(1)
@Slf4j
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<BaseResponse<Void>> handleRuntimeException(RuntimeException e) {
        ...
        if (e.getMessage().contains("warranty") || e.getMessage().contains("claim")) {
            BaseResponse<Void> response = BaseResponse.error("Warranty operation failed", "WARRANTY_OPERATION_ERROR");
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
        }
        
        if (e.getMessage().contains("invalid status")) {
            BaseResponse<Void> response = BaseResponse.error("Invalid warranty status", "INVALID_WARRANTY_STATUS");
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
        }
